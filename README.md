# Programación Web I — SIS-0300

**Carrera:** Ingeniería de Sistemas  
**Facultad:** Ingeniería  
**Universidad:** Universidad Privada Domingo Savio — Tarija, Bolivia  
**Semestre:** Quinto

---

## Descripción

Materiales de laboratorio para la asignatura Programación Web I. El módulo enseña a construir aplicaciones web completas usando React con TypeScript y Supabase como backend, a través de un proyecto incremental que evoluciona durante todas las clases.

---

## Stack tecnológico

| Herramienta     | Uso                                     |
| --------------- | --------------------------------------- |
| React + Vite    | Framework frontend y bundler            |
| TypeScript      | Tipado estático                         |
| Supabase        | Base de datos, autenticación y API REST |
| Tailwind CSS    | Estilos utilitarios                     |
| React Router v6 | Navegación entre páginas                |
| Recharts        | Reportes y gráficos                     |

---

## Proyecto del módulo

A lo largo del módulo se construye una **aplicación de películas (CineApp)** de forma incremental. Cada día agrega una capa nueva sobre lo anterior — desde componentes estáticos hasta una aplicación completa con autenticación, CRUD, rutas protegidas y reportes.

---

## Estructura del módulo

| Día                            | Tema                                              | Fase                   |
| ------------------------------ | ------------------------------------------------- | ---------------------- |
| [Día 0](./dia-00-typescript/)  | TypeScript esencial                               | Prerequisito           |
| [Día 1](./dia-01-componentes/) | Componentes, TSX y props                          | Fase 1 — React         |
| [Día 2](./dia-02-usestate/)    | Estado local con useState                         | Fase 1 — React         |
| [Día 3](./dia-03-listas/)      | Listas, .map() y composición                      | Fase 1 — React         |
| [Día 4](./dia-04-useeffect/)   | useEffect, Tailwind CSS y custom hooks            | Fase 1 — React         |
| [Día 5](./dia-05-supabase/)    | Supabase — configuración y primera consulta       | Fase 2 — Base de datos |
| [Día 6](./dia-06-crud/)        | CRUD completo (crear, leer, actualizar, eliminar) | Fase 2 — Base de datos |
| [Día 7](./dia-07-router/)      | React Router v6 — rutas públicas y protegidas     | Fase 3 — Rutas         |
| [Día 8](./dia-08-auth/)        | Supabase Auth — registro, login y rutas reales    | Fase 4 — Autenticación |

---

## Cómo usar este repositorio

Cada carpeta contiene la guía de laboratorio del día en formato `.md`. Para acceder a una guía hacé clic en el día correspondiente en la tabla de arriba o navegá directamente a la carpeta.

No se requiere instalar nada para leer las guías — se pueden ver directamente en GitHub.

---
